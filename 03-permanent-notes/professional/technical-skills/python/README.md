# Python Programming

**Purpose:** Core Python knowledge from fundamentals through advanced patterns and professional practices.

## Contents

Topics to build:
- [ ] Fundamentals (syntax, data types, control flow)
- [ ] Data Structures (lists, dicts, sets, tuples)
- [ ] Functions & Modular Code
- [ ] Object-Oriented Programming (OOP)
- [ ] Standard Library Modules
- [ ] Testing & Debugging
- [ ] Professional Practices (packaging, virtual environments)
- [ ] Code Quality (linting, formatting, type hints)

## Quick Reference

| Topic | Status | Last Updated |
|-------|--------|--------------|
| TBD | | |

## Learning Path

1. **Fundamentals** - Basic syntax and data types
2. **Data Structures** - Working with collections
3. **Functions** - Modular, reusable code
4. **OOP** - Building larger systems
5. **Standard Library** - Built-in tools
6. **Professional Skills** - Production-ready code

## How This Fits In

- **Part of:** [Technical Skills](../README.md)
- **Supports:** [Professional - AI Engineering](.././profession../professional/ai-engineering/README.md) and all technical projects
- **Resources:** [Literature Notes](../../../02-literature-notes/README.md)

## Next Steps

- Migrate Python learning from Notion
- Organize by skill level and topic
- Create practical exercises and projects

## Related Areas

- [Professional - AI Engineering](.././profession../professional/ai-engineering/README.md) - Applied Python
- [Software Engineering](../software-engineering.md) - Broader practices
- [Projects](../../../05-projects/README.md) - Apply knowledge

---

*Last updated: 2025-11-03*
